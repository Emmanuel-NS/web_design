...
let watchB=document.getElementById("watch")
let currentIndex = 0;
const changeBackground=()=>{
    container.style.background = `linear-gradient(rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 1)), url(${movieData[currentIndex].backdrop_path}) center/cover no-repeat`;
    title.innerText=movieData[currentIndex].title
    desc.innerText=movieData[currentIndex].overview
    rating.innerText=movieData[currentIndex].vote_average
    release.innerText=movieData[currentIndex].release_date
    watchB.dataset.bg = movieData[currentIndex].poster_path;
...
watchB.addEventListener('click', function (e) {

    e.preventDefault()
    window.location.href = 'nowshowing.html' + `?bg:${watchB.dataset.bg}`;
})
...
a.map(ele=>{ele.addEventListener('click', function (e) {
    e.preventDefault()
    let c = e.target
